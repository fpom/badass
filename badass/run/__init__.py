...
class Test(_Test):
    ...
    def __init__(self, text):
        ...
        if self.NUM == 1:
            self.lang.cleanup(self.project_dir / "src", self.project_dir / "itw")

    def __enter__(self):
        copytree(self.project_dir / "src", self.test_dir / "src", dirs_exist_ok=True)
        self.repo.update()
        return self
    ...
    def has(self, signature, declarations=None):
        return self.check(
            self.lang.decl(signature, declarations),
...
_diag2status = {"info": PASS, "warning": WARN, "error": FAIL}
                        ...
                        with _AllTest(test, text=text, details=details) as sub:
                            for diag, message, pos, before, after in info:
                                sub.add(
                                    _diag2status[diag],
                                    text=(
                                        f"`[{pos.path}:{pos.line}:{pos.col}]` {message}"
                                    ),
                                    details=(
                                        f"`{before.replace(' ', '§')}`"
                                        f"**`{after.replace(' ', '§')}`**"
